...
public class SellerOrderController {
    ...
    /*@GetMapping
    public ResponseEntity<ResponseDTO> getOrdersBySeller(@RequestHeader("Authorization") String authorizationHeader) {
        List<OrderDTO> orders = sellerOrderService.getOrdersBySellerId(authorizationHeader);
    ...
    public ResponseEntity<ResponseDTO> getOrderDetails(@PathVariable int orderId, @RequestHeader("Authorization") String authorizationHeader) {
        List<OrderDetailDTO> orderDetails = sellerOrderService.getOrderDetailsByOrderId(orderId, authorizationHeader);
        if (orderDetails != null) {
            return new ResponseEntity<>(new ResponseDTO(HttpStatus.OK.value(), "Order details fetched successfully", orderDetails), HttpStatus.OK);
        } else {
            return new ResponseEntity<>(new ResponseDTO(HttpStatus.NOT_FOUND.value(), "Order not found or not belonging to seller", null), HttpStatus.NOT_FOUND);
        }
    }*/
